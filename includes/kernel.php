...
set_error_handler(function ($errNo, $errStr, $errFile, $errLine) {
    ...
    if (DB::isConnected(DB_AOWOW))
        DB::Aowow()->query(
            ...
            $errFile,
            $errLine,
            CLI ? 'CLI' : ($_SERVER['QUERY_STRING'] ?? ''),
            empty($_POST) ? '' : http_build_query($_POST),
            User::$groups,
            $errStr
        );
...
if (!CLI) {
    ...
    if (!DB::isConnected(DB_AOWOW) || !DB::isConnected(DB_WORLD) || !Cfg::get('HOST_URL') || !Cfg::get('STATIC_URL')) (new GenericPage())->maintenance();
